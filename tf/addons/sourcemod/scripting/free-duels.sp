...
Handle c_EnableType[4]    = {INVALID_HANDLE, ...};
Handle cvarEnabled        = INVALID_HANDLE;
Handle c_Tag              = INVALID_HANDLE;
Handle c_EnableClass      = INVALID_HANDLE;
Handle c_EnableGodMod     = INVALID_HANDLE;
Handle c_EnableHeadShot   = INVALID_HANDLE;
Handle c_HeadShotFlag     = INVALID_HANDLE;
Handle c_Immunity         = INVALID_HANDLE;
Handle c_ClassRestriction = INVALID_HANDLE;
Handle c_GodModFlag       = INVALID_HANDLE;
...
static int TimeLeftOptions[10]               = {1, 2, 5, 10, 15, 20, 30, 45, 60, 120};
static int AmountOfKillOptions[12]           = {1, 2, 3, 4, 5, 10, 15, 20, 50, 75, 100, 150};
...
public Action EventPlayerSpawn(Handle hEvent, const char[] strName, bool bHidden) {
	...
	TFClassType restrict = view_as<TFClassType>(g_Duel[iClient].ClassRestrict);
	if(g_Duel[iClient].Enabled && restrict != TFClass_Unknown && TF2_GetPlayerClass(iClient) != restrict) {
		TF2_SetPlayerClass(iClient, restrict, false);
...
public Action EventPlayerDeath(Handle hEvent, const char[] strName, bool bHidden) {
	...
	if( GetConVarBool(c_EnableHeadShot) == true && g_Duel[iClient].HeadShot == true) {
		...
		if(headshot == false) return Plugin_Continue;
	}
	...
	if (g_Duel[iKiller].Challenger == iClient && g_Duel[iKiller].Enabled) {
		...
		if(g_Duel[iKiller].Enabled &&  g_Duel[iKiller].Type != 3 ) {
			...
			if(g_Duel[iKiller].Score == 0) {
			    EndDuel(iKiller, g_Duel[iKiller].Type);
			}
		}
		...
		if(g_Duel[iAssister].Enabled &&  g_Duel[iAssister].Type != 3 ) {
			...
			if(g_Duel[iAssister].Score == 0) {
			    EndDuel(iAssister, g_Duel[iAssister].Type);
			}
		}
	}
...
public Action OnTakeDamage(int iVictim, int &iAttacker, int &iInflictor, float &fDamage, int &iDamagetype) {
    /* TODO: the attacker and inflictor arguments are reversed compared to
       SDKHooks_TakeDamage. This may be an error.
       https://sourcemod.net/new-api/sdkhooks/SDKHooks_TakeDamage */

    /* If the victim isn't a valid client, then it is an engineer's
       building. Get it's owner and recheck that they are valid. */
	if (!IsValidClient(iVictim)) {
		iVictim = GetEntPropEnt(iVictim, Prop_Send, "m_hBuilder");
	}

	/* If the victim hurt themselves, or one of the entities involved
	   isn't a valid client, then we can ignore this event. */
	if (iAttacker == iVictim || !IsValidClient(iVictim) || !IsValidClient(iAttacker)) {
		return Plugin_Continue;
	}

	bool victim_challenged = g_Duel[iVictim].Challenger != iAttacker;
	bool attacker_challenged = g_Duel[iAttacker].Challenger != iVictim;

	/* TODO: What does the 1 mean? */
	bool victim_godmod = g_Duel[iVictim].Enabled && g_Duel[iVictim].GodMod == 1;
	bool attacker_godmod = g_Duel[iAttacker].Enabled && g_Duel[iAttacker].GodMod == 1;

	/* Stop the damage from happening if  */
	if ((victim_challenged || attacker_challenged) && (victim_godmod || attacker_godmod)) {
...
public Action EventSappedObject(Handle hEvent, const char[] strName, bool bHidden)
{
	int iSpy = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	int iEngi = GetClientOfUserId(GetEventInt(hEvent, "ownerid"));
	int iSapper = GetEventInt(hEvent, "sapperid");
...
public void OnGameFrame()
{
	...
	for(int iClient = 1; iClient <= MaxClients; iClient++ )
		if(IsValidClient(iClient) && g_Duel[iClient].Enabled)
		{
			...
			} else if(g_Duel[iClient].HideSprite) {
	...
	if(!g_Duel[iClient].Enabled)
	{
		if(!g_Duel[iClient].Type)
		{
			...
			if(iteam == 2 || iteam == 3) {
				int nbr = 0;
				for(int i = 1; i < MaxClients ; i++)	 {
				    /* Create an array if valid players (players + bots) */
					Player[i-1] = 0;
...
public void DuelPanel1(Handle menu, MenuAction action, int iClient, int args)
{
...
public void DuelOptionAnswer(Handle menu, MenuAction action, int Player1, int args) {
	if (action == MenuAction_Cancel) {
		...
		if(GetConVarInt(c_ClassRestriction) > 0) {
			...
			if(GetConVarInt(c_ClassRestriction) == 2) {
			    /* MaxClass Plugin */
				if(!StartReadingFromTable()) {
				    /* error while reding file  */
					SetConVarInt(c_ClassRestriction, 0);
					LogMessage("[Duel] Error while reading MaxClass config file. Now duel_classrestrict = 0 ");
				}
			} else {
				for (int i=1; i<=9; i++) {
					if(GetConVarInt(c_ClassRestriction) == 1) {
					    /* Class restrict plugin */
		...
		if(args == 1) {
		...
		} else if(args == 2 && g_Duel[Player1].Type == 2) {
		    /* Time */
			int i = 0;
		...
		} else if(args == 2 && g_Duel[Player1].Type == 3) {
		    /* Amount */
			int i = 0;
			...
			if(args == 3) {
				...
				if(IsClientInGame(g_Duel[Player1].Challenger)) {
					...
					if(IsFakeClient(g_Duel[Player1].Challenger)) {
...
bool StartReadingFromTable()
{
	...
	char mapname[32];
	int MaxClass[MAXPLAYERS][MAXTEAMS][MAXCLASSES];
	...
	do {
		...
		if (MaxPlayers[TFClass_Engineer] == -1) {
			MaxPlayers[TFClass_Engineer] = KvGetNum(kv, "engineer", -1);
		}
		...
		if (StrContains(buffer,"-") == -1) {
			...
			/* Make "1" and "5" into integers. Check boundries,
		    see if does not go out of the array limits. */
...
public void ChallengerMenuAnswer(Handle menu, MenuAction action, int Player2, int args) {
	if (action == MenuAction_Cancel) {
...
void LoadDuel(int Player2) {
	...
	TFClassType p2restrict = view_as<TFClassType>(g_Duel[Player2].ClassRestrict);
	TFClassType p2challenger = TF2_GetPlayerClass(g_Duel[Player2].Challenger);
	TFClassType p2class = TF2_GetPlayerClass(Player2);

	if(g_Duel[Player2].ClassRestrict != 0) {
	    /* Load class restriction */

		if(p2restrict != p2class) {
		    /* Player2 */
			TF2_SetPlayerClass(Player2, p2restrict, false);
			TF2_RespawnPlayer(Player2);
		}

		if(p2restrict != p2challenger) {
		    /* Player1 */
			TF2_SetPlayerClass(g_Duel[Player2].Challenger, p2restrict, false);
			TF2_RespawnPlayer(g_Duel[Player2].Challenger);
		}
	}

	if(g_Duel[Player2].GodMod) {
	    /* Load Godmod */
		if(p2class == TFClass_Engineer) {
...
public Action Hook_SetTransmit(int entity, int iClient) {
	if(EntRefToEntIndex(g_Duel[iClient].CSprite) == entity && !g_Duel[iClient].HideSprite) {
		return Plugin_Continue;
	}

	if(EntRefToEntIndex(g_Duel[g_Duel[iClient].Challenger].CSprite) == entity && !g_Duel[g_Duel[iClient].Challenger].HideSprite) {
		return Plugin_Continue;
	}
...
public bool isGoodSituation(int iClient, int Player2) {
	...
	if(g_Duel[Player2].Enabled) {
        /* too late ! iClient Player2 already in duel ... */
		CPrintToChat(iClient,"%t", "IsInDuel", Player2);
		ResetPlayer(iClient);
		return false;
	} else if (g_Duel[iClient].Enabled) {
	    /* you are already in duel ... */
		CPrintToChat(iClient,"%t", "InDuel");
		ResetPlayer(iClient);
		return false;
...
void GetSafeClientData(int iClient) {
	...
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "'", "");		// Fix player name for DB
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "<?PHP", "");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "<?php", "");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "<?", "");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "?>", "");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, "<", "[");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, ">", "]");
	ReplaceString(PlayerInfo, MAX_LINE_WIDTH, ",", ".");
	...
	GetClientAuthId(iClient, AuthId_Steam2, PlayerInfo, MAX_LINE_WIDTH);
	if (StrEqual(PlayerInfo, "") || StrEqual(PlayerInfo, "STEAM_ID_PENDING") || StrEqual(PlayerInfo, "STEAM_ID_LAN")) {
...
public void TopDuelPanel(Handle menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_End) {
		CloseHandle(menu);
	}
}
...
public int Native_IsDuelRestrictionClass(Handle plugin, int numParams) {
	...
	return g_Duel[iClient].ClassRestrict != 0;
}
